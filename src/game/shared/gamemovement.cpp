...
void CGameMovement::CheckParameters( void )
{
		 ...
		 player->GetMoveType() != MOVETYPE_OBSERVER )
	{
		...
		if ( maxspeed != 0.0 )
		{
			mv->m_flMaxSpeed = MIN( maxspeed, MAX(mv->m_flMaxSpeed, 450) ); // Remove the speed cap! Finally, we can go faster than 320 units per second! Whee!
																			// Thanks to https://forums.sourceruns.org/t/hl2-bunnyhop-restriction/2845
		}
...
void CGameMovement::AirMove( void )
{
	...
	if (player->IsLongJumpExpired()){
		mv->m_vecVelocity /= 2; // cripple player in all directions
		if (mv->m_vecVelocity.z <= 0){
			mv->m_vecVelocity.z *= 2; // but keep their speed the same if they're falling
		}
		player->SetSuitLongJump(false);
		player->SetLongJumpExpired(false);
	}
	TryPlayerMove();
...
// Perform a long jump by getting our view angles and changing that into a velocity vector. Distance is meant to be 700 units (plus elevation).
void CGameMovement::LongJump(void){
	QAngle viewAngles = mv->m_vecViewAngles;
	vec_t xVector = cosf(viewAngles.y * M_PI / 180);
	vec_t yVector = sinf(viewAngles.y * M_PI / 180);
	float scaleFactor = 1;
	// reduce horizontal movement as we look upwards, give a little forward boost for max upwards look
	if (viewAngles.x > -20) { viewAngles.x = 0; } // more leniency if you're looking up to 20 degrees upwards
	if (viewAngles.x < 0){
		scaleFactor = MIN((95.0f + viewAngles.x) / 89.0f, 1);
	}
	mv->m_vecVelocity = Vector(xVector * 700 * scaleFactor, yVector * 700 * scaleFactor, MAX(250, (viewAngles.x/45) * -250));
	m_bIsLongJumping = true;
	player->SetSuitLongJump(true);
}
...
bool CGameMovement::CheckJumpButton( void )
{
	...
	if (gpGlobals->curtime < m_fLastDuckTime + GAMEMOVEMENT_LJUMP_DELAY && player->CanLongJump()){
		LongJump();
		mv->m_nOldButtons |= IN_JUMP;
		return true;
	}
...
void CGameMovement::CheckFalling( void )
{
	...
	if ( !IsDead() && player->m_Local.m_flFallVelocity >= PLAYER_FALL_PUNCH_THRESHOLD && !m_bIsLongJumping)
	{
		bool bAlive = true;
		float fvol = 0.5;
	...
	else if (m_bIsLongJumping){
		PlayerRoughLandingEffects(0.85);
		player->SetSuitLongJump(false);
	}
...
void CGameMovement::Duck( void )
{
	...
	if (buttonsChanged & IN_DUCK){
		m_fLastDuckTime = gpGlobals->curtime;
	}
