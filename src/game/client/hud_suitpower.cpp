...
void CHudSuitPower::OnThink( void )
{
	...
	bool longJumpActive = pPlayer->IsLongJumpActive();
	int activeDevices = (int)flashlightActive + (int)sprintActive + (int)breatherActive + (int)longJumpActive;
...
void CHudSuitPower::Paint()
{
	...
	if ( m_iActiveSuitDevices )
	{
		...
		if (pPlayer->IsLongJumpActive())
		{
			tempString = g_pVGuiLocalize->Find("#Valve_Hud_LJUMP");
			...
			{
				surface()->DrawPrintText(L"LONG JUMP", wcslen(L"LONG JUMP"));
			}
			ypos += text2_gap;
		}
	}
