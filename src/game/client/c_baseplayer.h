...
public:
	...
	int				m_nNumMorphineCount;
	int				GetMorphineCount(void) {return m_Local.m_nNumMorphineCount;}
	...
	bool						CanLongJump() const		{ return m_bCanLongJump; }
...
public:
	int m_StuckLast;
	// these next few ones are for gamemovement so we can expire long jump but gamemovement is shared so this dumb fuckin' hack works for now
	void SetSuitLongJump(bool isTrue){}
	bool IsLongJumpExpired(){ return false; }
	void SetLongJumpExpired(bool isTrue){}
...
private:
	...
	float			m_flMaxspeed;
	bool			m_bCanLongJump = false; // FIXME: this is fucking stupid, the only reason the copy of long jumping code is here in client
	// is so the damned code can compile correctly
