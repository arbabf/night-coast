...
CSuitPowerDevice SuitDeviceLongJump(bits_SUIT_DEVICE_LJUMP, 50.0f);
...
void CHL2_Player::HandleSpeedChanges( void )
{
	...
	if (m_fTurnOffEnergyPoint != -1 && gpGlobals->curtime >= m_fTurnOffEnergyPoint){
		SetMovement(true);
	}
	if (m_bHasLongJumped){
		SuitPower_AddDevice(SuitDeviceLongJump);
	}
	else{
		SuitPower_RemoveDevice(SuitDeviceLongJump);
	}
	...
	if( IsSuitEquipped() )
	{
		bWantWalking = (m_nButtons & IN_WALK) && !IsSprinting() && !(m_nButtons & IN_DUCK) && m_bWalkEnabled;
	}
	else
	{
		bWantWalking = true;
...
void CHL2_Player::EnableWalk(bool bEnable){
	if (!bEnable && IsWalking()){
		StopWalking();
	}
	m_bWalkEnabled = bEnable;
}

ConVar sk_energymodule_time("sk_energymodule_time", "15");

void CHL2_Player::SetMovement(bool eEnable){
	EnableSprint(eEnable);
	EnableWalk(eEnable);
	SetMaxSpeed(HL2_NORM_SPEED + 260 * !eEnable); // 450 if movement overload, 190 if not
	if (!eEnable){
		m_fTurnOffEnergyPoint = gpGlobals->curtime + sk_energymodule_time.GetFloat();
	}
	else {
		m_fTurnOffEnergyPoint = -1;
	}
}
